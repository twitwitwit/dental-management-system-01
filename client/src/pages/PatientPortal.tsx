...
const tabs = ["overview", "appointments", "profile", "chart"] as const;
    ...
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:flex-row">
        <nav className="flex gap-1 overflow-x-auto rounded-xl border bg-card p-1 lg:block lg:h-fit lg:w-48 lg:shrink-0">
          {(["overview", "appointments", "profile", "chart"] as Tab[]).map(tab => (
            <button key={tab} type="button" onClick={() => onTab(tab)} className={`whitespace-nowrap rounded-lg px-3 py-2 text-left text-sm capitalize transition-colors lg:block lg:w-full ${activeTab === tab ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}>{tab === "overview" ? "Overview" : tab === "chart" ? "Dental chart" : tab}</button>
          ))}
        </nav>
...
export default function PatientPortal() {
  ...
  return <PortalShell activeTab={tab} onTab={setTab} onLogout={() => { void auth.logout(); setLocation("/patient/login"); }} name={patient.firstName}><>{tab === "overview" && <Overview onTab={setTab} />}{tab === "appointments" && <AppointmentsTab />}{tab === "profile" && <ProfileTab patient={patient} />}{tab === "chart" && <ChartTab />}</></PortalShell>;
}
